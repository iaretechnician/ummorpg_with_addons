...
// ===================================================================================
[Serializable]
public partial struct ExtendedQuest
{
    public int hash;
    ...
    // -------------------------------------------------------------------------------
    // ExtendedQuest (Constructor)
    // -------------------------------------------------------------------------------
    public ExtendedQuest(Scriptable_Quest data)
    {
    ...
    public Tools_Requirements questRequirements { get { return data.questRequirements; } }
    public Scriptable_Quest data { get { return Scriptable_Quest.All[hash]; } }
    public string name { get { return data.name; } }
    public bool showRewards { get { return data.showRewards; } }
    public RewardItem[] acceptItems { get { return data.acceptItems; } }
    public bool removeAtCompletion { get { return data.removeAtCompletion; } }
    public QuestReward[] questRewards { get { return data.questRewards; } }
#if _iMMOWORLDEVENTS
    public WorldEventTemplate worldEvent { get { return data.worldEvent; } }
    public int worldEventModifier { get { return data.worldEventModifier; } }
#endif
    public Npc[] visitTarget { get { return data.visitTarget; } }
#if _iMMOPVP
    public PvpTarget[] pvpTarget { get { return data.pvpTarget; } }
#endif
    public KillTarget[] killTarget { get { return data.killTarget; } }
    public GatherTarget[] gatherTarget { get { return data.gatherTarget; } }
...
#if _iMMOFACTIONS
    public FactionModifier[] factionModifiers { get { return data.factionModifiers; } }
#endif
#if _iMMOEXPLORATION
    public Exploration_Area[] exploreTarget { get { return data.exploreTarget; } }
#endif
#if _iMMOHARVESTING
    public HarvestTarget[] harvestTarget { get { return data.harvestTarget; } }
#endif
#if _iMMOCRAFTING
    public CraftTarget[] craftTarget { get { return data.craftTarget; } }
#endif
#if _iMMOFACTIONS
    public Quest_Faction factionRequirement { get { return data.factionRequirement; } }
#endif
#if _iMMOCHEST
    public LootTarget[] lootTarget { get { return data.lootTarget; } }
#endif
    ...
    public string ToolTip(int[] gathered, int explored = 0, bool factionReqMet = false, int level = 0)
    {
        ...
        {
            tmpString = "";
            foreach (RewardItem reward in questRewards[0].rewardItem)
            {
...
#if _iMMOTITLES
            if (questRewards[0].title != null)
                tip.Replace("{REWARDTITLE}", "<b><color=yellow> * Earn Title</color></b>\n - " + questRewards[0].title.displayName.ToString());
#endif
    ...
    private string TooltipSnippet_PvpTarget(int level)
    {
        ...
        if (pvpTarget.Length > 0)
        {
            ...
            for (int j = 0; j < pvpTarget.Length; ++j)
            {
                ...
                if (pvpTarget[j].type == PvpTarget.pvpType.OtherGuild ||
                    pvpTarget[j].type == PvpTarget.pvpType.OtherParty ||
                    pvpTarget[j].type == PvpTarget.pvpType.OtherRealm)
                {
                    tmpString += "   " + pvpNot;
                }

                if (pvpTarget[j].type == PvpTarget.pvpType.MyParty ||
                    pvpTarget[j].type == PvpTarget.pvpType.MyGuild ||
                    pvpTarget[j].type == PvpTarget.pvpType.MyRealm)
                {
                    tmpString += "   " + pvpMy;
                }
                ...
                if (pvpTarget[j].type == PvpTarget.pvpType.MyGuild ||
                    pvpTarget[j].type == PvpTarget.pvpType.OtherGuild)
                {
                    tmpString += pvpGuild;
                }

                if (pvpTarget[j].type == PvpTarget.pvpType.MyRealm ||
                    pvpTarget[j].type == PvpTarget.pvpType.OtherRealm)
                {
                    tmpString += pvpRealm;
                }
    ...
    private string TooltipSnippet_VisitTarget()
    {
        ...
        if (visitTarget.Length > 0)
        {
            ...
            {
                string done = visitedTarget.Contains(npc.name.GetStableHashCode()) ? " [ok]" : " [x]";
    ...
    private string TooltipSnippet_ExploreTarget(int explored = 0)
    {
        ...
        if (exploreTarget.Length > 0)
        {
            ...
            foreach (Exploration_Area area in exploreTarget)
            {
                tmpString += " - " + area.name + "\n";
            }
        }
