...
public partial class UI_SkillbarMobile : MonoBehaviour
{
    public GameObject panel;
    public UI_SkillbarSlotMobile slotPrefab;
    ...
    void Update()
    {
        ...
        if (player != null)
        {
            ...
            for (int i = 0; i < player.skillbar.slots.Length; ++i)
            {
                ...
                UI_SkillbarSlotMobile slot = content.GetChild(i).GetComponent<UI_SkillbarSlotMobile>();
                ...
                if (skillIndex != -1)
                {
                    ...
                    slot.button.onClick.SetListener(() => {
                        if (!player.target || player.target.health.current == 0 || player.target == player)
                        {
                            player.targetButtonPressed = true;
                        }
                        // try use the skill or walk closer if needed
                        ((PlayerSkills)player.skills).TryUse(skillIndex);
                    });
