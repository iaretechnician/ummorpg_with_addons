...
public class SkillEffect_Projectile : SkillEffect
{
    ...
    [HideInInspector] public Skill_BaseDamage parentSkill; // set by skill
    ...
    public void Apply()
    {
        ...
        if (caster.target is Player && caster is Player && ((Player)caster).Tools_SameCheck((Player)caster.target, parentSkill.affectSelf, parentSkill.affectPlayers, parentSkill.affectOwnParty, parentSkill.affectOwnGuild, parentSkill.affectOwnRealm, parentSkill.reverseTargeting) ||
			(caster.target is Monster && parentSkill.affectMonsters ) ||
			(caster is Monster && caster.target is Monster && parentSkill.affectMonsters) ||
            ...
            if (parentSkill.castRadius.Get(skillLevel) > 0)
        {
            ...
            if (caster is Player)
                targets.AddRange(((Player)caster).Tools_GetCorrectedTargetsInSphere(target.transform, parentSkill.castRadius.Get(skillLevel), false, parentSkill.affectSelf, parentSkill.affectOwnParty, parentSkill.affectOwnGuild, parentSkill.affectOwnRealm, parentSkill.reverseTargeting, parentSkill.affectPlayers, parentSkill.affectNpcs, parentSkill.affectMonsters, parentSkill.affectPets));
            else
                targets.AddRange(caster.Tools_GetCorrectedTargetsInSphere(target.transform, parentSkill.castRadius.Get(skillLevel), false, parentSkill.affectSelf, parentSkill.affectOwnParty, parentSkill.affectOwnGuild, parentSkill.affectOwnRealm, parentSkill.reverseTargeting, parentSkill.affectPlayers, parentSkill.affectNpcs, parentSkill.affectMonsters, parentSkill.affectPets));
