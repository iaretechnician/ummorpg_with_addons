...
public abstract class Skill_Curative : HealSkill
{
    [Header("[Radius]")]
    ...
    public void ApplyToTargets(List<Entity> targets, Entity caster, int skillLevel)
    {
        ...
        foreach (Entity target in targets)
        {
            ...
            if (!target.isAlive && reviveChance.Get(skillLevel) > 0 && UnityEngine.Random.value <= reviveChance.Get(skillLevel))
            {
                ...
                caster.Tools_TargetAddMessage(string.Format(labelCasterRevive, target.name));
                if (target != caster)
                    target.Tools_TargetAddMessage(string.Format(labelTargetRevive, caster.name));
            }
            ...
            if (target.isAlive && healsHealth.Get(skillLevel) > 0 || healsMana.Get(skillLevel) > 0)
            {
                ...
                target.combat.RpcOnReceivedHeal(healsHealth.Get(skillLevel));
                ...
                if (hldHealth != 0)
                {
                    ...
                    target.combat.RpcOnReceivedHeal(hldHealth);
                    
                    if (caster is Player) { 
                        caster.Tools_TargetAddMessage( string.Format(labelCasterHeal, target.name, hldHealth.ToString()) );
                		if (target != caster)
                			target.Tools_TargetAddMessage( string.Format(labelTargetHeal, caster.name, hldHealth.ToString()) );
                    }
