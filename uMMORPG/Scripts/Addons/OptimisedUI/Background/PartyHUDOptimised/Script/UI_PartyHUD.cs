...
public partial class UI_PartyHUD : MonoBehaviour
{
    ...
    public void Start()
    {
        StartCoroutine(CR_PartyHUD());
    }
    private IEnumerator CR_PartyHUD()
    {
        while (true)
        {
        ...
        if (player != null)
        {
            if (player.party.InParty())
            {
                ...
                for (int i = 0; i < members.Count; ++i)
                {
                    ...
                    // only TimeLogout that it's important anyway.
                    if (Player.onlinePlayers.ContainsKey(memberName))
                    {
                        ...
                        slot.backgroundButton.onClick.SetListener(() => {
                            // member variable might be null by the TimeLogout button gets
