...
public partial class UI_Target : MonoBehaviour
{
    ...
    public void Start()
    {
        StartCoroutine(CR_Target());
    }
    private IEnumerator CR_Target()
    {
        while (true)
        {
    ...
    void UpdateTarget()
    {
        ...
        if (player != null)
        {
            ...
            if (target != null && target != player)
            {
                ...
                // trade button
#if _iMMOPVP
                if (target is Player && target.Realm == player.Realm)
#else
                if (target is Player)
#endif
                {
                ...
                // guild invite button
#if _iMMOPVP
                if (target is Player targetPlayer && player.guild.InGuild() && target.Realm == player.Realm)
#else
...
#if _iMMOPVP
                if (target is Player targetPlayer2 && target.Realm == player.Realm)
#else
                if (target is Player targetPlayer2)
#endif
                {
