...
public partial class Tools_Requirements
{
    ...
    public PrestigeClassTemplate[] allowedPrestigeClasses;
#endif
    ...
    public virtual bool checkRequirements(Player player)
    {
        ...
        if(player.playerHarvesting)
            valid = player.playerHarvesting.HasHarvestingProfessions(harvestProfessionRequirements, requiresAllHarvestingProfessions) ? valid : false;
#endif
#if _iMMOCRAFTING
...
#if _iMMOTRAVEL
		valid = string.IsNullOrWhiteSpace(requiredTravelrouteName) || (!string.IsNullOrWhiteSpace(requiredTravelrouteName) && player.playerTravelroute.travelroutes.Any(t => t.name == requiredTravelrouteName)) ? valid : false;
#endif
...
#if _iMMOGUILDUPGRADES
		if (minGuildLevel > 0)
			valid = player.guild.InGuild() && player.playerGuildUpgrades.guildLevel >= minGuildLevel ? valid : false;
#endif
